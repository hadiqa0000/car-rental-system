...
public class Main {
    ...
    private static void browseCarsForCustomer() {
        ...
        while (true) {
            ...
            for (int i = 0; i < brandArray.length; i++) {
                int count = carsByBrand.get(brandArray[i]).size();
                System.out.printf("%d. %s (%d car%s available)%n",
                        i + 1, brandArray[i], count, count == 1 ? "" : "s");
            }
            ...
            int brandChoice;
            try {
                brandChoice = Integer.parseInt(input);
            } catch (NumberFormatException e) {
            ...
            if (brandChoice < 1 || brandChoice > brandArray.length) {
    ...
    private static void viewMyRentals() {
        ...
        List<Rental> rentals
                = rentalManager.getRentalsForCustomer(currentCustomer.getId());
